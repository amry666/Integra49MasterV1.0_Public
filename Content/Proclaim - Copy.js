var unit = localStorage.satunit;
localStorage.lastpage = 1;
$('#judulhal').html(localStorage.judulhal);
$('#judulmenu').html(localStorage.judulmenu);
...
//FILTER DROPDOWN
function Select2_bysp(spquery, element, placeholder) {
    $.ajax
        ({
            url: "../CRUD/R_Proc",
            type: "GET",
            ...
            success: function (response) {
                ...
                var html = "";
                for (var x = 0; x < data2.length; x++) {
                    html += "<option  value='" + data2[x].ID + "'>" + data2[x].KODE + " " + data2[x].NAMA + "</option>";
                }
                $(element).empty();
                $(element).append(html);
            }
...
function Select2tree_byspauth(spquery, element, placeholder) {
    $.ajax
        ({
            url: "../CRUD/R_Proc_Auth",
            type: "GET",
            data: spquery + "&t=" + $.cookie('t'),
            cache: !0,
            success: function (response) {
